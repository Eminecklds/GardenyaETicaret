...
namespace GardenyaGirisimciKadinlar.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ViewBag.Resim = db.Users.Where(x => x.Email == model.Email).Select(x => x.Resim).FirstOrDefault();
        ...
        public ActionResult Register()
        {
            ...
            {
                ViewBag.GelenRole = "User";
            }
            else { 
            var routeValue = RouteData.Values["id"];
            if (routeValue.ToString() == "1")
            {
                ViewBag.GelenRole = "Girisimci";
            }
            else
            {
                ViewBag.GelenRole = "User";
            ...
            //ViewBag.GelenRole = routeValue;
            RegisterViewModel rw = new RegisterViewModel();
            rw.UserRole = ViewBag.GelenRole;
            return View(rw);
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model, HttpPostedFileBase file)
        {
            ...
            var gelenRole =model.UserRole;

            var user = new ApplicationUser { UserName = model.Ad, Email = model.Email, PhoneNumber = model.Tel, Ad = model.Ad, Soyad = model.Soyad, UserRole = gelenRole,Resim=model.Resim,UyelikTarihi = model.UyelikTarihi };
            ...
            var result = await UserManager.CreateAsync(user, model.Password);
                ...
                if (!RoleManager.RoleExists("Girisimci"))
                {
                    var role = new Microsoft.AspNet.Identity.EntityFramework.IdentityRole();
                    role.Name = "Girisimci";
                    RoleManager.Create(role);
                ...
                await this.UserManager.AddToRoleAsync(user.Id, gelenRole);
